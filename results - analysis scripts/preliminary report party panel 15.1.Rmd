---
title: "Party Panel 15.1 Preliminary Report"
...
```{r pp-setup, echo=FALSE}
waveNumber <- '15.1';
surveyId <- 652829;
startDate <- '1st of July 2015';
embargoLiftedDate <- '1st of July 2017';
...
Party Panel `r waveNumber` - Preliminary Report
=====================================

## Introduction

Party Panel is a Dutch panel study where determinants and beliefs underlying health behaviors in the Dutch nightlife are mapped. The second wave started on the `r startDate` and explored the determinants of carrying, wearing, and buying earplugs for hearing protection.
...
This document is confidential. It is illegal to copy or distribute it in any form. An embargo of 2 years rests upon all Party Panel datasets. The wave described in this document started on the `r startDate`, and therefore, the dataset of this Party Panel wave will be released under CC-BY-NC-SA (see http://creativecommons.org/licenses/by-nc-sa/4.0/) on the `r embargoLiftedDate`.
...
ggSave <- function(filename=default_name(plot), plot = last_plot(),
                   device="png", height=16, width=16, units="cm",
                   type="cairo-png", ...) {
  ggsave(filename=filename, plot=plot, device=device,
         height=height, width=width, units=units, type=type, ...);
}
...
ggSave(recruitmentInfo$plots$relativeDropout,
       file = file.path(workingPath, "dropout.png"),
       height=6);
